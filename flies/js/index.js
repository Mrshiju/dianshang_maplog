const MODILE = "release";
const DEAUTFUL_CID = "63278423232";
const DEAUTFUL_MOBILE = "13111182711";
const DEAUTFUL_URL_REDIRECT = 'http://sms-yx-api-http.hanlonglyt.com/labelson/click.do';
const DEAUTFUL_URL_SHOP = 'http://shop.hanlonglyt.com';
const DEAUTFUL_URL_LOG_CLICK = 'http://sms-yx-api-http.hanlonglyt.com/order/clicklog';
const DEAUTFUL_URL_SUBMIT = 'http://sms-yx-api-http.hanlonglyt.com/order/saveOrder';
pcid = DEAUTFUL_CID;
...
function subOrder(callback, cid, pid, goodsid, userName, mobile, address, money, unit, num, comm, source, des) {
  ...
  var data = `cid =${cid} &pid =${pid } &userName = ${userName} & mobile = ${
    mobile
  } & address = ${
    address
  } & money = ${
...
window.onload = function() {
  ...
  function() {
    ...
    var list_new2 = list_new.substring(1, list_new.length - 1).split('\',\'');
    ...
    var img2 = img.substring(1, img.length - 1).split('\',\'');
    var testFn = {
      list: list_new2,
      headLength: 20,
      scrollHeight: 36,
      ...
      init: function() {
        this.wrapper && this.container && this.load(function() {
          if (this.list && this.list.length) {
            if (this.container.innerHTML = this.createHTML(!0, 0) + this.createHTML(!1, this.list.length), false) {
      ...
      updateUI: function() {
        var t = 0;
        if (this.totalLoop % this.list.length == 0 && this.totalLoop > 0) for (; t < this.list.length;) this.container.querySelector("li").style.top = (this.list.length + t) * this.scrollHeight + "px",
        ...
        for (t = 0; t < e.length; t++) {
          ...
          n == this.scrollHeight ? e[t].style.opacity = 0 : n == 2 * this.scrollHeight ? e[t].style.opacity = .9 : n == 3 * this.scrollHeight && (e[t].style.opacity = .3),
          e[t].style.top = n - this.scrollHeight + "px"
        }
      }
    };
    testFn.init()
